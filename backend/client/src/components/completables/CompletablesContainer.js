...
class CompletablesContainer extends Component {
  listCompletables = () => {
    return(
      !!this.props.localCompletables.map && this.props.localCompletables.map(completable => <CompletableCard key={completable.id} completable={completable} allCompletables={this.props.allCompletables} toggleCompletable={this.props.toggleCompletable}/>)
    )
  }
  render(){
    return(
